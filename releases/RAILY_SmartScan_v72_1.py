# SmartScan RAILY v72.1 — Fast Date Reader & Accessible Teaching Forms
...
def _read_taught_date(path, region, progress=None, fast=False,
    ...
    try:
        ...
        def attempts():
            ...
            # Expensive image work is lazy: only prepare a fallback being used.
            for mode in ("blue-ink", "line-clean", "threshold"):
                if mode == "blue-ink" and not CFG.get("handwritten_date_mode", True):
                    continue
                yield "exact", exact, mode, 0, digits, "Handwriting fallback"
            if CFG.get("handwritten_date_mode", True):
                # Retain blue-ink tightening for loose zones, but only here.
                blue_mask = image_variant(exact, "blue-mask")
                if blue_mask is not None:
                    for name, ink_crop in _candidate_date_subcrops(exact, blue_mask=blue_mask):
                        if name == "auto-ink-tight":
                            crops[name] = ink_crop
                            yield name, ink_crop, "blue-ink", 0, digits, "Handwriting fallback"
                            break
                rotation_mode = "blue-ink" if blue_mask is not None else "line-clean"
                for angle in (90, 270, 180):
                    yield "exact", exact, rotation_mode, angle, digits, "Handwriting fallback"
        ...
        def read(crop, mode, angle, config, stage, name):
            ...
            try:
                fragment = pytesseract.image_to_string(
                    image, lang=CFG.get("ocr_language", "eng"), config=config, timeout=5.0
                ).strip()
            except Exception:
                return None, None
            if fragment:
        ...
        for name, crop, mode, angle, config, stage in attempts():
            ...
            if not accepted and key and calls >= DATE_FAST_OCR_LIMIT and calls < limit:
                confirm_config = "--psm 7" if config == digits else digits
        ...
        log(f"Date reader {'fast' if fast else 'full'}: page {page_index + 1}, "
            f"{calls} OCR calls, {time.perf_counter() - started:.3f}s, limit {limit}")
