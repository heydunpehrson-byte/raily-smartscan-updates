# SmartScan RAILY v71 — Railroad AI Dispatch & Full-Page Visual Learning
...
def ocr_file(path):
    """Read first two pages. Reuse text layer when useful; OCR only when needed."""
    p = Path(path)
    ...
    if p.suffix.lower() == ".pdf":
        ...
        try:
            for page_no, page in enumerate(doc):
                ...
                if len(re.findall(r"[A-Za-z0-9]", embedded)) < 140:
                    pix = page.get_pixmap(
...
def printed_structure_fingerprint(path, fallback_text=""):
    ...
    img = _load_page_image(path, 0, scale=1.35)
    printed = []
    ...
    if img is not None:
        try:
            ...
            for raw, conf in zip(data.get("text", []), data.get("conf", [])):
                ...
                # Variable values are intentionally suppressed.
                ...
                if len(token) < 3:
                    continue
                if token in STRUCTURE_STOPWORDS:
                    continue
...
def _candidate_date_subcrops(crop_rgb):
...
def _ocr_date_region(path, region, progress=None, _allow_recovery=True):
    """Read a taught date SEARCH ZONE.

    v55 makes the user's rectangle forgiving: SmartScan tests several tighter
    crops inside the taught zone and chooses the best valid date. v65 can
    also try a few tightly bounded nearby variants when scanner alignment drifts.
    """
    ...
    if progress:
        progress(
            "Preparing forgiving date search zone..."
        )
    ...
    x0 = max(
        0.0,
        float(region.get("x0", 0.0)) - padding
    )
    y0 = max(
        0.0,
        float(region.get("y0", 0.0)) - padding
    )
    x1 = min(
        1.0,
        float(region.get("x1", 1.0)) + padding
    )
    y1 = min(
        1.0,
        float(region.get("y1", 1.0)) + padding
    )
    ...
    candidates = _candidate_date_subcrops(
        zone_rgb
    )

    # Evidence across separate internal crops. Agreement across crops is a
    # powerful signal because they all came from the user's date zone.
    global_votes = {}
    results = []
    ...
    for index, (
        ...
        if progress:
            progress(
                f"Trying tighter date crop {index}/{len(candidates)}: {candidate_name}"
            )

        result = _ocr_one_date_candidate(
            candidate_name,
            candidate_rgb,
            page_index,
            progress=progress,
            progress_prefix=(
                f"Date crop {index}/{len(candidates)}"
            )
        )
        ...
        # Strong within-crop consensus counts more than a lone OCR hit.
        weight = (
            3
            if result["strong"]
            else 1
        ...
        global_votes[key] = (
            global_votes.get(key, 0)
            + weight
        )

        # A single strong candidate is enough to stop early. This avoids
        # needless OCR on every possible trim once the date is clear.
        if result["strong"]:
            break

        # Two separate weak sub-crops agreeing on the same date also suffice.
        if global_votes.get(key, 0) >= 2:
            break

    if not global_votes:
        # v63 fallback: the taught zone may intentionally include a label,
        # account number, amount, border, etc. Read it as a multi-line block
        # instead of forcing the user to redraw a tiny date-only rectangle.
        block_result = _date_zone_text_prepass(
            zone_rgb,
            page_index,
            progress=progress,
            fast=False
        ...
        if _allow_recovery:
            recovered = _recover_date_region(path, region, progress=progress)
            if recovered:
                return recovered
        return None
    ...
    details = (
        f"page {page_index + 1}, "
        f"best crop {strongest['candidate']}, "
        f"zone score {best_score}, rotation "
        + (
            ", ".join(
                f"{angle}°"
    ...
    if progress:
        progress(
            f"Date found using {strongest['candidate']}."
        )
...
def _quick_ocr_date_region(path, region):
    """Small automatic date pass for normal sorting.

    This intentionally does far less work than the full Teach Date OCR:
      • modest render scale
      • a few safe sub-crops
      • blue/line-clean variants only
      • digits-first OCR
      • early exit

    Full handwriting OCR remains available asynchronously inside Teach.
    """
    p = Path(path)
    ...
    # Slightly smaller padding than the full forgiving search keeps automatic
    # processing focused and fast.
    padding = min(
        0.012,
        float(
            CFG.get(
    ...
    for candidate_name, candidate in candidates:
        ...
        for variant_name, prepared_img in prepared[:2]:
            if prepared_img.width < 720:
                scale = min(
                    2.7,
                    max(
                        1.35,
            ...
            # Most scans are upright. Try 0 first, then the two common
            # sideways orientations, and 180 last.
            for angle in (
                0,
                90,
                270,
                ...
                found = _quick_date_from_fragment(
                    fragment
                )
                ...
                for dt in found:
                    ...
                    # One valid date inside an exact taught date zone is useful;
                    # two agreeing fast passes are enough for the strongest
                    # quick confidence and stop immediately.
                    if votes[key] >= 2:
                        return (
                            dt,
                            98,
                            (
                                f"quick crop {candidate_name}, "
                                f"{variant_name}, {angle}°"
                            )
                        )

    if not votes:
        # v63: keep normal sorting fast. Only after the existing one-line OCR
        # fails do we spend up to two OCR calls on a multi-line statement box.
        block_result = _date_zone_text_prepass(
            zone,
            page_index,
            fast=True
        ...
        if block_result:
            dt, confidence, region_text, details = block_result
            return (
                dt,
    ...
    if (
        ...
        if profile:
            ...
            for region in regions:
                result = _quick_ocr_date_region(path, region)
                if result:
...
def detect_family_date(category, family, text, path=None, include_learned=True):
...
class App:
    def __init__(self, root):
        ...
        self.note(f"RAILY SmartScan v{APP_VERSION} Clean Foundation — Railroad AI Dispatch is ready.")
    ...
    def _build_raily_dispatch(self, parent):
        ...
        outer = tk.Frame(parent, bg=dispatch_bg, padx=10, pady=10)
        ...
        track_frame = tk.Frame(outer, bg=panel_bg, padx=10, pady=6)
        ...
        preview_panel = tk.Frame(
            body, bg=panel_bg, padx=10, pady=10,
        ...
        ai_panel = tk.Frame(
            body, bg=panel2_bg, padx=12, pady=10,
            highlightbackground=steel, highlightthickness=1
        )
        ...
        route_box = tk.Frame(ai_panel, bg="#0F171A", padx=10, pady=8)
        route_box.grid(row=9, column=0, columnspan=2, sticky="ew", pady=(10, 5))
        tk.Label(
        ...
        tk.Label(
            ...
            font=("Segoe UI", 11, "bold"), wraplength=480, justify="left"
        ...
        switch = tk.Frame(lower, bg=panel_bg, padx=8, pady=8)
        ...
        self.raily_activity = tk.Text(
            feed, height=6, wrap="word", bg="#0A0F11", fg="#D8E0E3",
    ...
    def open_raily_memory_manager(self):
        ...
        def add_entity_tab(title, kind):
            ...
            tree = ttk.Treeview(frame, columns=("name", "count", "last"), show="headings")
            ...
            ttk.Button(frame, text="Remove Selected", style="Action.TButton", command=delete_selected).pack(anchor="e")
        ...
        try:
            ...
            for i, name in enumerate(self.raily_stage_names):
                ...
                if i < self.raily_stage_index:
                    ...
                    fill = "#3D4A50"
                canvas.create_oval(x - 9, y - 9, x + 9, y + 9, fill=fill, outline="#D5DDE0", width=1)
                canvas.create_text(x, 53, text=name, fill="#D8E0E3", font=("Segoe UI", 8, "bold"))
        except Exception:
            pass
    ...
    def _update_raily_preview(self, path):
        try:
            ...
            if not p.exists():
                return
            img = render_document_preview(p, page_index=0, max_width=610, max_height=535)
            if img is None:
                return
    ...
    def start(self):
        ...
        if hasattr(self, "raily_stage_var"):
            self.raily_stage_var.set("RAILY is watching the yard for the next document")
    ...
    def teach_file(
        ...
        # v70: the old 920x~688 Teach window could become vertically
        # compressed on common 1366x768 / Windows-scaled laptops.  Tk then
        # squeezed wrapped labels into neighboring rows, which made words
        # appear jumbled together.  Give the Teach UI essentially the full
        # usable screen and maximize it on Windows when supported.
        width = min(
            1320,
            max(900, sw - 36)
        )
        ...
        # Reserve enough height for the document preview while keeping the
        # classification rows at their requested sizes.
        win.grid_columnconfigure(
            0,
            weight=1
        )
        win.grid_rowconfigure(
            4,
            weight=1,
            minsize=220
        )

        # Maximize AFTER the fallback geometry is set.  This keeps the
        # window usable on systems where 'zoomed' is not implemented.
        try:
            win.state("zoomed")
        except Exception:
            pass
        ...
        # Attention
        attention_holder = ttk.Frame(win)
        ...
        def show_page(index):
            ...
            try:
                img = render_document_preview(
                    p,
                    index,
                    max_width=max(520, width - 110),
                    max_height=max(220, height - 430)
                )
        ...
        def open_date_area_trainer():
            ...
            tsw = trainer.winfo_screenwidth()
            tsh = trainer.winfo_screenheight()

            tw = min(1360, max(940, tsw - 36))
            th = min(920, max(640, tsh - 58))
            tx = max(0, (tsw - tw) // 2)
            ty = max(0, (tsh - th) // 2)

            trainer.geometry(f"{tw}x{th}+{tx}+{ty}")
            trainer.minsize(min(880, tw), min(600, th))

            trainer.grid_columnconfigure(0, weight=1)
            trainer.grid_rowconfigure(2, weight=1, minsize=300)

            # v70: Date Profile is a teaching workspace, so use the full
            # available display instead of a small centered dialog.
            try:
                trainer.state("zoomed")
            except Exception:
                pass
            ...
            # Toolbar
            # ---------------------------------------------------------
            trainer_toolbar = ttk.Frame(trainer)
        ...
        def split_and_teach_pages():
            ...
            dw = min(650, max(560, dialog.winfo_screenwidth() - 120))
            dh = 360
            dx = max(20, (dialog.winfo_screenwidth() - dw) // 2)
            dy = max(20, (dialog.winfo_screenheight() - dh) // 2)
